...
`.env.example`를 `.env.local`로 복사해 채울 것. 테스트 프레임워크는 아직 없음 — 작업을 "끝났다"고 보고하기 전 최소 `npm run build` + `npm run lint`로 컴파일·정적 분석 통과 확인.
...
**`lib/anthropic/`**.
- `client.ts`: SDK singleton + `invokeAgent()` (JSON) / `streamAgent()` (SSE). **`cacheSystemAndTools: true`** 시 system prompt 마지막 block + tools 마지막 정의에 `cache_control: ephemeral` 부여 (claude-api skill 가이드). **부모 셸이 빈 `ANTHROPIC_API_KEY=` export 시 fallback으로 .env.local 직접 파싱** (Claude Code harness 환경 대응).
- `pricing.ts`: 모델별 토큰 가격(input/output/cache write/read) + `calculateCostUsd()`.

**`lib/openai/`** (Phase 3+).
- `embeddings.ts`: `text-embedding-3-small` + `dimensions=1024` (스키마 vector(1024) 호환). `embedOne` / `embedMany`(batch 96개). 동일 fallback 패턴.

**`lib/obsidian/`** — 옵시디언 vault 동기화 + 의미 검색 (Phase 3).
- `scanner.ts`: vault root 재귀 walk → `.md` 파일 list (`.obsidian`/`.trash`/dotfile 무시). `OBSIDIAN_VAULT_PATH` env 기반.
...
2. invoke route에 `Accept: text/event-stream`으로 fetch
3. upstream 이벤트 forward — `session` 이벤트로 sessionId/userMessageId 추가
4. `done`만 가로채 `chat_messages.insert` + assistantMessageId 추가해 emit
5. `lastMessageAt` 갱신

URL `?session=<uuid>`로 세션 재진입(히스토리 GET). 클라이언트는 `lib/sse/client.ts`의 `streamSseFetch`로 처리. `/today`(하영)도 동일 패턴이지만 `chat_sessions` 비영속 (Phase 1 단순 채팅).
...
- 작업 규칙은 글로벌 행동 가이드라인(`~/.claude/CLAUDE.md`) + `AGENTS.md` + 이 파일을 함께 따른다. 충돌 시 우선순위는 AGENTS.md > 이 파일 > 글로벌.
